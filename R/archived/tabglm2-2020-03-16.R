...
#' factor levels.
#' @param output Character string specifying type of output. Choices are 'viewer'
#' (output to RStudio Viewer), 'kable', 'matrix', and 'html' (write HTML file to
#' working directory).
...
tabglm2 <- function(fit,
                    ...
                    decimals = 2,
                    formatp.list = NULL,
                    html.filename = paste("table1-", Sys.Date(), ".html", sep = "")) {
  ...
  if (! output %in% c("viewer", "kable", "matrix", "html")) {
    stop("The input 'output' must be set to 'viewer', 'kable', 'matrix', or 'html'.")
  }
  ...
  # If output = "viewer", format and call kable() and kable_styling()
  if (output %in% c("viewer", "kable")) {

    slashes <- paste(rep("&nbsp; ", indent.spaces), collapse = "")
    df$Variable <- gsub(pattern = spaces, replacement = slashes, x = df$Variable, fixed = TRUE)
    df.printed <- df %>% kable(row.names = FALSE, escape = FALSE) %>% kable_styling(full_width = FALSE)
    if (output == "viewer") df.printed
    return(df.printed)

  }

  # If output = "matrix", return as is
  if (output == "matrix") return(df)

  # If output = "html", print html file
  if (print.html) {
